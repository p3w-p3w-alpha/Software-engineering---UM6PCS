...
/**
 * repository for private message data access
 * handles direct messaging between users
 * might need to optimize teh conversation queries for performace
 */
@Repository
public interface PrivateMessageRepository extends JpaRepository<PrivateMessage, Long> {

    // custom query for finding all messages in a conversation between two users (non-deleted), ordered by time
    // needed this becuase we need bidirectional conversation lookup
    ...
    // custom query for finding all messages in a conversation with pagination - used in chat view
    @Query("SELECT m FROM PrivateMessage m WHERE m.deleted = false AND " +
           "((m.sender.id = :user1Id AND m.receiver.id = :user2Id) OR " +
           "(m.sender.id = :user2Id AND m.receiver.id = :user1Id)) " +
    ...
    // custom query for finding all unread messages for a user - for notifications
    ...
    // custom query for counting unread messages for a user - for badge counter
    @Query("SELECT COUNT(m) FROM PrivateMessage m WHERE " +
           "m.deleted = false AND m.read = false AND m.receiver.id = :userId")
    long countUnreadMessages(@Param("userId") Long userId);

    // custom query for counting unread messages from a specific user
    @Query("SELECT COUNT(m) FROM PrivateMessage m WHERE " +
