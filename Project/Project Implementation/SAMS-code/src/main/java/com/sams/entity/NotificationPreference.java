...
/**
 * NotificationPreference entity - user preferences for notifications
 * allows users to control what types of notifications they recieve
 *
 * one-to-one with user - each user has their own preference settings
 * the switch statement in isNotificationTypeEnabled is kind of ugly but works
 */
@Entity
...
public class NotificationPreference {
    ...
    // one preference setting per user - enforced by unique constraint
    @OneToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "user_id", nullable = false, unique = true)
    private User user;

    // global setting - disable all email notifications if false
