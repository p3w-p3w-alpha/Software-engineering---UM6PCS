...
/**
 * repository for user connection data access
 * handles student/faculty networking and connections
 * needed this becuase we support social networking features
 */
@Repository
public interface ConnectionRepository extends JpaRepository<Connection, Long> {

    // custom query for finding connection between two users (in either direction)
    // needed this becuase we need bidirectional lookup
    ...
    // custom query to check if connection exists between two users - for validation
    ...
    // custom query to check if one user has blocked another - for privacy enforcement
    @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM Connection c WHERE " +
    ...
    // custom query for finding all accepted connections for a user - used in network page
    @Query("SELECT c FROM Connection c WHERE " +
    ...
    // custom query for finding all pending connection requests sent by a user
    @Query("SELECT c FROM Connection c WHERE " +
           "c.status = 'PENDING' AND c.requester.id = :userId " +
           "ORDER BY c.createdAt DESC")
    List<Connection> findPendingRequestsSent(@Param("userId") Long userId);

    // custom query for finding all pending connection requests received by a user - for notifications
    @Query("SELECT c FROM Connection c WHERE " +
           "c.status = 'PENDING' AND c.receiver.id = :userId " +
           "ORDER BY c.createdAt DESC")
    List<Connection> findPendingRequestsReceived(@Param("userId") Long userId);

    // custom query for finding all blocked users for a user - might need to optimize performace
    @Query("SELECT c FROM Connection c WHERE " +
           "c.status = 'BLOCKED' AND " +
    ...
    // custom query for finding all connections by status for a user
    @Query("SELECT c FROM Connection c WHERE " +
           "c.status = :status AND " +
           "(c.requester.id = :userId OR c.receiver.id = :userId) " +
           "ORDER BY c.createdAt DESC")
    ...
    // custom query for counting accepted connections for a user - for dashboard stats
    @Query("SELECT COUNT(c) FROM Connection c WHERE " +
           "c.status = 'ACCEPTED' AND " +
           "(c.requester.id = :userId OR c.receiver.id = :userId)")
    long countAcceptedConnections(@Param("userId") Long userId);

    // custom query for counting pending requests received by a user
    @Query("SELECT COUNT(c) FROM Connection c WHERE " +
           "c.status = 'PENDING' AND c.receiver.id = :userId")
    long countPendingRequestsReceived(@Param("userId") Long userId);

    // custom query for finding rejected connections for a user
    @Query("SELECT c FROM Connection c WHERE " +
