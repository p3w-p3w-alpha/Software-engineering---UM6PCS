...
/**
 * StudentDegreeProgress entity - tracks student progress toward degree completion
 * links student to their degree program and tracks completion status
 *
 * the auto-calculation stuff in PrePersist/PreUpdate is really handy
 * completion percentage updates automatically which saves a lot of manual work
 */
@Entity
...
public class StudentDegreeProgress {
    ...
    // the student whos progress we're tracking
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "student_id", nullable = false)
    private User student;

    // the degree program teh student is pursuing
    @ManyToOne(fetch = FetchType.LAZY)
