<!--
  ...
  shows unread count badge and handles websocket notifications
  took forever to get teh dropdown positioning right lol
-->

<template>
  <div class="relative" ref="container">
    <!-- Notification Bell Button - shows badge when unread -->
    <button
      @click="togglePanel"
...
// component state - managing panel visibility and filters
const isOpen = ref(false)
const filter = ref('all')  // can be 'all' or 'unread'
const notifications = ref([])
const container = ref(null)  // for click outside detection

// calculate unread count - this occured in multiple places so made it computed
...
// lifecycle hooks - setup and cleanup
onMounted(async () => {
...
function subscribeToNotifications() {
  websocketService.subscribeToNotifications(authStore.userId, (notification) => {
    notifications.value.unshift(notification)  // add new notification at top

    // Show browser notification if permitted - works for now
