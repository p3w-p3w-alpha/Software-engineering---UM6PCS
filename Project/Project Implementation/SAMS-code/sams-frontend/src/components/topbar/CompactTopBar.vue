<!--
  CompactTopBar - modern compact navigation bar at top of page
  shows logo, search, notifications, theme toggle, and user menu
  took forever to get all teh dropdown menus and keyboard shortcuts working right
-->

<template>
  <header class="topbar">
    <div class="topbar-container">
      <!-- Left Section: Logo & Branding with breadcrumbs -->
...
import NotificationPanel from '@/components/NotificationPanel.vue'  // notification dropdown
import {
  AcademicCapIcon,
  MagnifyingGlassIcon,
  BellIcon,
...
// Refs for UI state - lots going on here
const userMenuRef = ref(null)  // for click outside detection
const searchInputRef = ref(null)
const showUserMenu = ref(false)
const showNotifications = ref(false)
const showSearch = ref(false)  // modal search overlay
const searchQuery = ref('')
const isDarkMode = ref(true)  // theme state - TODO: fix later to persist

// Computed user info from auth store
...
// current page title for breadcrumbs - definately could be better
...
// Search results (simple page search) - filters available pages
...
// toggle dark/light theme - works for now
const toggleTheme = () => {
  isDarkMode.value = !isDarkMode.value
  document.documentElement.classList.toggle('dark', isDarkMode.value)
}

// open search modal with keyboard focus
const openSearch = () => {
  showSearch.value = true
  nextTick(() => {
    searchInputRef.value?.focus()  // auto-focus search input
  })
}
...
// handle user logout - this occured multiple times so centralized
...
onUnmounted(() => {
  ...
  document.removeEventListener('click', handleClickOutside)  // prevent memory leaks
})
</script>
