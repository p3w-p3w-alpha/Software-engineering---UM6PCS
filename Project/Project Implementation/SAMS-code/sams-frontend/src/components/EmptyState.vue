<!--
  EmptyState - shows nice empty state when no data available
  handles different illustration types: no-data, search, inbox
  the animations took a while to get smooth - works for now
-->

<template>
  <div class="empty-state" :class="{ 'compact': compact }">
    <div class="illustration-wrapper">
      <!-- SVG illustrations - custom icons or default based on type -->
...
<script setup>
// props for customizing teh empty state display
defineProps({
  title: {
    type: String,
  ...
  customIcon: {
    type: Boolean,
    default: false  // allows using custom icon slot
  },
  compact: {
    type: Boolean,
    default: false  // smaller version for tight spaces
  }
})

// emits action event when button is clicked
defineEmits(['action'])
</script>
