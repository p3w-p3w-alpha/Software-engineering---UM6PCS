            ...
            <div class="space-y-6">
              ...
              <div class="flex items-center gap-4">
                <div class="relative">
                  ...
                  <div
                    v-if="profilePicturePreview"
                    class="absolute -bottom-1 -right-1 bg-green-500 rounded-full p-1"
                  >
...
const profile = ref({
  ...
  profilePicture: null // Base64 or URL for profile picture
})
...
async function loadUserProfile() {
  try {
    ...
    if (userId) {
      ...
      if (savedPicture) {
        profilePicturePreview.value = savedPicture
      }
    }
  } catch (error) {
...
async function handleFileUpload(event) {
  ...
  if (!validTypes.includes(file.type)) {
    toast.add({
      ...
      detail: 'Please select a JPG, PNG, GIF, or WebP image',
      life: 3000
    })
    return
  }
  ...
  try {
    ...
    reader.onload = async (e) => {
      ...
      profilePicturePreview.value = base64

      // Store in local storage for persistence
      const userId = authStore.userId
      localStorage.setItem(`profilePicture_${userId}`, base64)

      // Try to upload to server (may fail if endpoint doesn't exist)
      try {
        await api.uploadProfilePicture(userId, file)
      } catch (uploadError) {
        // Server upload failed, but local storage is saved - this is expected in dev
      }
...
async function saveProfile() {
  ...
  try {
    ...
    toast.add({
      ...
      detail: error.userMessage || error.response?.data?.message || 'Failed to update profile. Please try again.',
      life: 3000
    })
  } finally {
    saving.value = false
